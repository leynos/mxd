...
async fn process_stream<F, T>(
    ...
    T: Preamble,
{
    let preamble_result = if let Some(duration) = preamble_timeout {
        match timeout(duration, read_preamble::<_, T>(&mut stream)).await {
            Ok(result) => result,
            Err(_) => Err(DecodeError::Io {
                inner: io::Error::from(io::ErrorKind::TimedOut),
                additional: 0,
            }),
        }
    } else {
        read_preamble::<_, T>(&mut stream).await
    };
    ...
    match preamble_result {
        ...
        Err(err) => {
            if let Some(handler) = on_failure.as_ref()
                && let Err(e) = handler(&err, &mut stream).await
            {
                error!(
            ...
            // If no failure handler is provided, emit a fallback log for visibility.
