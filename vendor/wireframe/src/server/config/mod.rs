    ...
    S: ServerState,
{
    ...
    builder_setter!(
        /// Configure an optional timeout for decoding the connection preamble.
        ///
        /// When set, the server will abort preamble decoding if it does not
        /// complete within the specified duration. The connection is closed
        /// after invoking any registered preamble failure handler.
        preamble_timeout, preamble_timeout, timeout: Duration => Some(timeout)
    );
