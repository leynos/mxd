...
CREATE TABLE file_nodes (
    ...
    CHECK (
        (is_root AND node_type = 'folder' AND parent_id IS NULL AND alias_target_id IS NULL
         AND object_key IS NULL AND size = 0 AND NOT is_dropbox)
        OR
        (NOT is_root AND parent_id IS NOT NULL)
    ),
    CHECK (
