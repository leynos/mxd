...
/// Returns `true` when both the `permissions` and `user_permissions` tables
/// exist and carry their expected auto-index entries.
async fn sqlite_permission_schema_is_aligned(conn: &mut DbConnection) -> TestResult<bool> {
    ...
    .await?;
    if tables != vec!["permissions", "user_permissions"] {
        return Ok(false);
    }
    ...
    .await?;
    Ok(user_permission_indices
        .iter()
        .any(|name| name == "sqlite_autoindex_user_permissions_1"))
}

/// Returns `true` when `news_bundles` has the expected columns and indices.
async fn sqlite_bundle_schema_is_aligned(conn: &mut DbConnection) -> TestResult<bool> {
...
///
/// The predicate queries `PRAGMA index_list` through `sqlite_names` and checks
/// that `idx_articles_category`, `idx_articles_first_child_article`,
...
pub(super) async fn sqlite_article_indices_are_present(
...
async fn sqlite_news_schema_is_aligned(conn: &mut DbConnection) -> TestResult<bool> {
    if !sqlite_bundle_schema_is_aligned(conn).await? {
        return Ok(false);
    }
    if !sqlite_article_indices_are_present(conn).await? {
        return Ok(false);
    }
    ...
    .await?;
    if !["idx_categories_bundle", "idx_news_categories_unique"]
        .iter()
...
/// Returns `true` when the full aligned schema is present, including
/// permission tables, news schema, and the root-category uniqueness constraint.
///
...
/// `verify_root_category_names_are_unique_with_constraint_insert`.
async fn sqlite_aligned_schema_is_valid(conn: &mut DbConnection) -> TestResult<bool> {
    if !sqlite_permission_schema_is_aligned(conn).await? {
        return Ok(false);
    }
    if !sqlite_news_schema_is_aligned(conn).await? {
        return Ok(false);
    }
...
async fn sqlite_fresh_migration_creates_aligned_schema() -> TestResult<()> {
    ...
    assert!(
        sqlite_aligned_schema_is_valid(&mut conn).await?,
